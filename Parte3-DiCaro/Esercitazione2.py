...
def guess_by_definitions(definitions, GENUSES=7, MIN_PROF=5, SHOW=-1):
    nouns = get_noun_occurrencies(definitions)
    sorted_nouns = sort_dictionary_by_value(nouns)
    genuses = sorted_nouns[:GENUSES]
...
if __name__ == "__main__":
    ...
    with open(definitions_path) as file:
        ...
        for line in list(tsv_file)[1:]:
            filter_definition_by_size(
                door_definitions, line[1], minimum_definition_length
            )
            filter_definition_by_size(
                ladybug_definitions, line[2], minimum_definition_length
            )
            filter_definition_by_size(
                pain_definitions, line[3], minimum_definition_length
            )
            filter_definition_by_size(
                blurriness_definitions, line[4], minimum_definition_length
            )
    ...
    for definition_collection in definitions:
        i = 0
        for guess in guess_by_definitions(definition_collection, SHOW=10):
            print(guess)
            if i < 5:
                print(guess[0].definition())
                print()
            i += 1
        print("----------NEXT--------")
        print()
